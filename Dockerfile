...
RUN mkdir /state-in-memory && chown flink:flink /state-in-memory  # workaround for https://github.com/docker/compose/issues/3270
RUN mkdir /state-rocks-db && chown flink:flink /state-rocks-db
